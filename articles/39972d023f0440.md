...
topics: ["designsystem", "figma", "storybook", "biome", "mcp"]
published: false
publication_name: "moshjp"
---

デザインシステムの整備・運用をしているが、いざコーディングエージェントにUI実装をお願いしてみるとこれじゃない感がすごい。みたいな経験をしている方は少なくないと思います。
...
@[speakerdeck](35d4cbefb0f04785b84fed6ec1626a73)

## MOSHのデザインシステムの構成

以下のようになっており、それぞれ乗っかっている場所や管理する人が異なります。

| 領域 | 置き場所 | 管理している内容 | メンテナンス |
| --- | --- | --- | --- |
| デザインデータ | Figma | デザイントークン、コンポーネント定義 | デザイナー |
| ガイドライン | Notion | 用途・使い分け、UXライティング、アクセシビリティ | デザイナー |
| Storybook | コードベース | 社内向け共通UIコンポーネントのカタログ | エンジニア |

## 実装とデザインの乖離

Angularで構築されたフロントエンドの構成からReactへ移行する際に[shadcn/ui](https://ui.shadcn.com/)をそのまま使うという選択をしました。
...
同義のコンポーネントが各チームで実装されたり強制的にスタイルを上書きするなどのハックを生み出します。
そうすると共通コンポーネントにデザインを当てて、各機能に反映しようとした時にコンポーネントが使われていなかったり上書きしているスタイルが原因で意図した通りにスタイルが当たらないといった二次被害が発生します。

:::message
MOSHでは共通コンポーネントはプラットフォームの1つと定義しており、オーナーはプラットフォームエンジニアです。
プラットフォームはプロダクト開発とは違うラインで開発されており、社内で広く使われることを目的としています。
プロダクトエンジニアからのフィードバックや要望を取りまとめて反映していくなど、通常のプロダクト開発で行われるようなサイクルがプラットフォームにも存在します。
:::
...
これはコーディングエージェントを使っても同様で、例えばFigmaやNotionをMCPとして自身のツールに連携していたとしてもこのコンポーネントのFigma/NotionのURLはこれ、という情報がなければエージェントが正しい情報にたどり着くことはありません。
...
Figmaにデザインデータがあれば多少それっぽくはなったりするのですが、それでもコンポーネントの使い方が間違っていたり使って欲しいコンポーネントが使われなかったりするなど、都度手直しが必要なことが多いです。
...
| 課題 | 打ち手 |
| --- | --- |
| 情報の置き場所が異なる | コンポーネントにFigma/Notionを紐づける |
| 実装とデザインの乖離 | `design-context` / `design-review` |
| 同じ指摘の再発 | Linterのカスタムルール |
| AIの生成精度の低さ | よくあるレイアウトを自動生成する |
...
こうすることでエージェントがStorybook MCP経由でコンポーネントの一覧を取得する際やコンポーネントの実装サンプルを見ようとしたタイミングでURLを取得することになります。
あとはMCPさえ接続できていれば正しい情報を取りに行ける状態になります。
ただし、これだけだと上記の挙動を毎回再現してくれることが保証されているわけではないのでSkillなどに落とし込む必要があります。
...
| スキル | 役割 | アウトプット |
| --- | --- | --- |
| `design-context` | 情報収集と実装方針の作成 | 実装方針 |
| `design-review` | 実装のレビュー、評価 | 修正提案 |
| `design-principles` | 判断の優先順位、コンポーネントに収まらない領域の定義 | なし(参照のみ) |

Gistで公開するにあたって内容を一般化していますが、基本的な方針は変わっていません。
https://gist.github.com/dachi023/30d46ca29c4fe4ad3dd8fdf18c51beaa
...
無いものは無いのでまずは正の状態を定義しましょう、とすることで無いから憶測で実装してしまうというSlop[^1]を回避するようにしています。

[^1]: AIが生成したそれっぽいけれど質の低い成果物のこと。ここでは定義が存在しない箇所をエージェントが憶測で埋めてしまい、一見動くが根拠のないコードが積み上がる状態を指しています。

### `/design-review`

実装後のセルフレビューとして用意しています。
ここでの指摘は必ずガイドライン等の根拠があることが必須としています。
指摘事項はMUST / SHOULD / IMO / nits / Qに分類していますが、これはこのSkill固有のものというよりはMOSHのレビュー方針に合わせている部分なので利用者が自身の環境に合わせてカスタマイズする部分です。

LintやVRTで検知できる問題については意図的に外しています。
また、フィードバックループの中で何度も指摘しているものはLinterのカスタムルールを作成する提案をさせるなど、AIによる指摘の揺らぎを極力減らす方向に持っていくよう設計しています。

:::details /design-review 実行結果(サンプル)
```plain
## Design Review 結果
...
なので、判断に迷った際や情報を埋める際の補助的な役割ですが、ここが漏れると実装不備が発生する可能性が高いので毎回参照させるようになっています。

個別のルールはJSDocにURLを記載し、それで拾えないものはここに記載するという構造です。

## Linterのカスタムルール

例えば色を指定する際にカラーコードが一緒だとしても `text-black` ではなく `text-primary` としましょう、といったルールです。
...
:::message
MOSHでは[Biome](https://biomejs.dev/)を採用しているのでGritQLでカスタムルールを作成しています。
:::
...
| ルール | 対象コンポーネント | 説明 |
| --- | --- | --- |
| `no-badge-utility` | `<Badge>` | `className`の`bg-*`を禁止。専用Propsで指定する |
| `no-heading-text-utility` | `<Heading>` | `className`の`text-*` / `font-*` / `leading-*` / `tracking-*`を禁止。専用Propsで指定する |
| `no-text-text-utility` | `<Text>` | `className` の `text-*` / `font-*` / `line-clamp-*` / `leading-*` / `break-*` / `truncate` / `whitespace-*`を禁止。専用Propsで指定する |
| `no-native-heading-tag` | すべて | `<h1>` 〜 `<h6>` の直接使用を禁止。`<Heading as="h1">` を使う |
| `no-native-paragraph-tag` | すべて | `<p>` の直接使用を禁止。`<Text>` を使う |
| `no-tailwind-bg-color` | すべて | Tailwind標準の色 `bg-*` と任意値(`bg-[#fff]` や `bg-[var(--x)]` など)を禁止 |
| `no-tailwind-border-color` | すべて | Tailwind標準の色 `border-*` と任意値 `border-[...]` を禁止 |

## よくあるレイアウトを自動生成する

一覧や詳細ページ、フォーム入力など目的によってレイアウトを固定したいケースが発生したため、npm scriptsに `add-route` コマンドを用意しました。
よくある画面のパターンを事前に実装しておき `add-route` 実行時にテンプレート一覧から使いたいものを選択させ複製するという挙動です。
実際の運用等を考慮し、パターンをimportして使うのではなくshadcnのように複製することで各機能ごとに調整できる余地を残しています。
なので複製後はあとはよろしく方式なのですが、少なくとも複製した時点ではデザインレビューを必要としないクオリティのものが生成されている状態になっています。

この仕組みは[Plop](https://plopjs.com/)を使って実現しています。
構成要素は以下の通りで、サンプル実装とジェネレーターを紐づけるための中間層としてregistryを定義しています。

| 要素 | 実体 | 役割 |
| --- | --- | --- |
| テンプレート | `app/components/examples/<name>/<name>.tsx` | ページコンポーネントのサンプル |
| レジストリ | `app/components/examples/registry.ts` | 選択肢の一覧(key / label / 説明 / テンプレートパス / コンポーネント名) |
| ジェネレータ | `plopfile.js` + `plop/route-from-example.js` | Plop用のgenerator実装。テンプレートを複製してrouteを新規追加する |

:::details generatorのコード
### registry.ts
...
MOSHの場合はReact Routerの[flatRoutes](https://reactrouter.com/how-to/file-route-conventions)を採用しています。ここはアプリケーションの構成によって処理が変わるので適宜置き換えてください。
合わせてテストファイルを生成していますがMOSHでは `bun:test` からimportしています、こちらも必要に応じてJestなどに置き換える等してください。
...
![ワンショットで生成した一覧画面](/images/39972d023f0440/generated-page.png)
*実際にはこんな画面は存在しないのですがデモとして載せるために作成しました*

## さいごに

MOSHの場合はFigmaとNotionという構成でしたが、組織によってどう管理しているかは変わるので適宜読み替えてもらえればと思います。
情報が一元化されていればもっと楽になるのはそうなのですが、デザイナーの負荷を無視してツールを変えるか？と言われるとそうではないし、既存の資産を捨てるのもやりすぎだと判断して今回のような意思決定をしています。

今はまだデザインに追従できていない状態なのでどうしてもデザイナーにFigma上でモックを用意してもらって、ということが発生してしまうのですがそれが解消されればモックやプロトタイプの作成は全部コード上でやる、コードが正の状態である、ということが出来るようになります。
コーディングエージェントに頼めばエンジニア以外も「こういう画面が作りたいんですけど」を表現しやすくなりますし、コードが実際にそこにあるので実装に転用しやすくなります。

## 宣伝

実装前にコンポーネントを解析、専用のSkillを使うことで実装のブレやミスを減らすツールを作り始めました。興味があれば覗いてみたり、使ってみたりしてみてください。フィードバックも大歓迎です。
https://yosegi-dev.github.io/yosegi/
